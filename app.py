...
<table><tr><th>Cas</th><th>Mots Ens.</th><th>Mots Élève</th><th>Densité Ens.</th><th>Densité Él.</th><th>US</th><th>P1</th><th>P2</th><th>P3</th></tr>"""
            ...
            for e in ENSEIGNANTS:
                ...
                body += f"<tr><td>{e}</td><td>{dve['ew'] or '—'}</td><td>{dve['lw'] or '—'}</td><td>{dve['pe']+'%' if dve['ew']+dve['lw'] else '—'}</td><td>{dve['pl']+'%' if dve['ew']+dve['lw'] else '—'}</td><td>{len(d4['noeuds'])}</td><td>{s4['pv']['P1']}</td><td>{s4['pv']['P2']}</td><td>{s4['pv']['P3']}</td></tr>"
            tt=tw+tl
            body += f"<tr style='font-weight:bold'><td>TOTAL</td><td>{tw}</td><td>{tl}</td><td>{round(1000*tw/tt)/10+'%' if tt else '—'}</td><td>{round(1000*tl/tt)/10+'%' if tt else '—'}</td><td>{tu}</td><td>—</td><td>—</td><td>—</td></tr></table>"
